...
data.barang.forEach((b, idx) => {
  semuaData.push({
    docId: docSnap.id,
    indexBarang: idx,
    originalIndex: semuaData.length, // index asli di semuaData
  ...
  tableHTML += `
    <tr data-idx="${b.originalIndex}">
      ...
      <td>${b.namaBarang}</td>
        ...
        <td>${b.status}</td>
        <td>
         <button class="status-btn" data-idx="${b.originalIndex}">Sudah Dicetak</button>
<button class="edit-btn" data-idx="${b.originalIndex}">Edit</button>
 </td>
      </tr>
    `;
  });
...
tableContainer.querySelectorAll('tr[data-idx]').forEach(row => {
...
// Tombol "Sudah Dicetak"
tableContainer.querySelectorAll('.status-btn').forEach(btn => {
  btn.addEventListener('click', async e => {
    e.stopPropagation();
    const index = Number(btn.dataset.idx); // pakai data-idx
    const b = semuaData[index];
    if (!b || b.status === "Sudah Dicetak") return;
    ...
    try {
      ...
      const newBarang = data.barang.map((item, idx) =>
        idx === b.indexBarang ? { ...item, status: "Sudah Dicetak" } : item
      );
